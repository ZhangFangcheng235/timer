...
function getLunarSecond(seconds) {
    ...
    if (seconds >= 1000) {
        ...
        if (mantissa >= 1 && mantissa < 10) {
            ...
            const decimalPart = Math.round((mantissa - integerPart) * 10);

            scientificNotation = digits[integerPart] + '.0x10^' + exponent;
        }

        // 处理小数部分
        let remainder = seconds % Math.pow(10, exponent);
        if (remainder === 0) {
            scientificNotation += '+' + digits[0]; // 整数情况
        } else {
            // 处理小数部分
            let fractionStr = '';
            ...
            if (fraction >= 1000) {
                ...
                fractionStr += digits[thousandsDigit] + thousands[thousandsDigit];
                fraction %= 1000;
            }
            ...
            if (fraction >= 100) {
                ...
                fractionStr += hundreds[hundredsDigit] || digits[hundredsDigit] + '百';
                fraction %= 100;
